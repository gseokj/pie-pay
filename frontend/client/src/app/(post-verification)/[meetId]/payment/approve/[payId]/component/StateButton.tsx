    ...
    <div className={styles.container}>
      ...
      <div className={styles.content}>
        <div className={styles.waitSection}>
          {payment?.participants[0].payAgree === 'wait' &&
          ...
          {payment?.participants[0].payAgree === 'deny' &&
            <button className={styles.button.afterinstead}>도움 요청하기👋</button>}
        </div>
        <div className={styles.agreeSection}>
          {payment?.participants[0].payAgree === 'agree' &&
            <p className={styles.tip}> TIP : 멤버를 선택해서 대신 결제 할 수 있어요!</p>}
          {payment?.participants[0].payAgree === 'agree' && <button className={styles.button.afterAgree}>승인완료</button>}
        </div>
      </div>
    </div>);
}