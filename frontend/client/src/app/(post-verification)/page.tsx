...
export default function Main() {
    ...
    const meetList: Meet[]|undefined = queryClient.getQueryData(['meetList', token]);
        ...
        return meetDataArray.sort((a, b) => {
            ...
            if (a.topFixed === b.topFixed) {
                ...
                if (a.lastPayDate && b.lastPayDate) {
                    const dateComparison = new Date(b.lastPayDate).getTime() - new Date(a.lastPayDate).getTime();
                ...
                const aCreatedAt = a.createdAt ? new Date(a.createdAt).getTime() : new Date().getTime();
                const bCreatedAt = b.createdAt ? new Date(b.createdAt).getTime() : new Date().getTime();
    ...
    if (typeof meets === 'undefined') {
        ...
        return (
            <>
                ...
                {typeof meets !== 'undefined' && meets.map((meet: Meet) => {
                    return (
                        <MeetGroup meet={meet} key={meet.meetId} updateIsFixed={() => updateStatus(meet.meetId)} />
                    )
                })}
