...
import {Meet, MeetData} from "@/model/meet";
import {Meet as Meets} from "@/model/meet/meets";
...
export default function MeetSetting({params}: Props) {
    ...
    const onSubmit = async () => {
        try {
            ...
            queryClient.setQueryData(['meetList', token], (oldData: Meets[]) => {
                const newData = oldData.map(data => {
                    if (data.meetId === Number(meetId)) {
                        return { ...data, meetName: meetName };
                    }
                    return data;
                });
                return newData;
            ...
            if (reader.readyState === 2) {
                ...
                try {
                    ...
                    queryClient.setQueryData(['meetList', token], (oldData: Meets[]) => {
                        const newData = oldData.map(data => {
                            if (data.meetId === Number(meetId)) {
                                return { ...data, meetImage: imageURL };
                            }
                            return data;
                        });
                        return newData;
    ...
    const onClickDelete = async () => {
        if (typeof token === 'string') {
            ...
            queryClient.setQueryData(['meetList', token], (oldData: Meet[]) => {
                const newData = oldData.filter(meet => meet.meetId !== Number(meetId));
                return newData
            });
            router.replace('/');
        } else {
